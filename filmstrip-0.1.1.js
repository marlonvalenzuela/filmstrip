    ...
    jPlug.Filmstrip = function (element, options) {
        ...
        */
        ...
        function getFrameInfo() {
            _instance.children().each(function (i) {
                var node = $(this);
                _frame.wdths.push(node.width());
                _frame.hghts.push(node.height());
                if (isH()) _frame.space = jPlug.utils.getPixels(node.css("margin-right"));
                    else _frame.space = jPlug.utils.getPixels(node.css("margin-bottom"));
            });
        }
        ...
        function create() {
            _instance.children().each(function (i) {
                var node = $(this);
                var myPosition = _frame.loc[i];
                node.css("position", "absolute");
                node.css((isH()) ? "left" : "top", myPosition + "px");
                addNextPosition(i);
            });
        }

        function addNextPosition(i) {
            var size = (isH()) ? _frame.wdths[i] : _frame.hghts[i];
            var nextPosition = (size + _frame.space) + _frame.loc[i];
            addFrameLocation(nextPosition);
        }
        ...
        */
        ...
        function isLastFrame () {
            if (getIndex()==(_frame.loc.length-1)-_settings.inView) return true;
                else false;
        }
